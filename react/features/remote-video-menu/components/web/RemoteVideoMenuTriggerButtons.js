...
class RemoteVideoMenuTriggerButton extends Component<Props> {
  ...
  _renderRemoteVideoMenu() {
    ...
    if (_isModerator) {
     ...
     buttons.push(
      <PinVideo key="PinVideo " participantID={participantID} />
    );
    ...
    if (_remoteControlState) {
      ...
      if (_remoteControlState === REMOTE_CONTROL_MENU_STATES.STARTED) {
        ...
        onRemoteControlToggle = () =>
          dispatch(requestRemoteControl(participantID));
      }
...
function _mapStateToProps(state, ownProps) {
  ...
  const _isRemoteControlSessionActive =
    participant?.remoteControlSessionStatus ?? false;
  ...
  if (
    _supportsRemoteControl &&
    ((!active && !_isRemoteControlSessionActive) ||
  ...
  return {
    _isAudioMuted:
      isRemoteTrackMuted(tracks, MEDIA_TYPE.AUDIO, participantID) || false,
    _isModerator: Boolean(
